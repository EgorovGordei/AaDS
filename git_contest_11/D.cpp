...
public:
    struct iterator
    {
        ...
        int _go_up(int current_node, int symbol)
        {
            int next_node = -1;
        ...
        iterator& operator++()
        {
            ...
            while (true)
            {
                ...
                if (!first && tr->trie[current_node].term > 0) break;
                first = false;

                int next_node = _go_up(current_node, symbol);
    ...
    void add_word(const std::string& word)
    {
        ...
        {
            if (trie[current_node].edge[letter - STARTING_SYMBOL] != -1)
            {
                current_node = trie[current_node].edge[letter - STARTING_SYMBOL];
            }
            else
            {
                trie.emplace_back(ALPHABET_SIZE, letter, current_node);
                trie[current_node].edge[letter - STARTING_SYMBOL] = trie.size() - 1;
